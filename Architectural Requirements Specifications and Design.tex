...
\begin{document}
	...
	\begin{titlepage}
		\begin{center}
			...
			\href{https://github.com/lindelo}{Keegan Ferrett - 15132847}\\[5mm]
...
The Point of Interest system will be implemented in 10 classes, making use of two of the classical design patterns, to help decouple the system from the rest of the application. The Facade and Factory design happens where choosen for this implementation.\\

\begin{itemize}
  \item The \textbf{Facade} design pattern was implemented with the LocationAccess and Location Manager (both inheriting from the LocationFacade class) as the Facades and having used RegularUser and AdminUser (both inheriting from User) as the Clients of the design pattern. The implementaion of this design pattern will happen managering the list of location easier for both the user and the admin and give certain functionality to each user group that the other should not have access to.
  \item The \textbf{Factory} design pattern was implemented using the LocationFactory class as the Factory and the Location class as the products of the design pattern. The purpose of including this design pattern is to hide the creation of locations to be saved from the users and to keep the references of Location objects away from the users.
\end{itemize}
